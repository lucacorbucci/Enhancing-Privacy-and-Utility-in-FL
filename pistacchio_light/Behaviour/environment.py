...
class Manage_Environment:
    ...
    def initialize_nodes(self, nodes: list, return_nodes=False) -> dict:
        ...
        for node_selected in nodes:
            ...
            if new_node.status == 1:
                ...
                logger.debug(
                    f"Information from envrionment: Node {node_selected} joined the environment."
                )
            else:
                if self.preferences["verbose"] >= 1:
                    logger.debug(
                        f"Information from envrionment: Client {node_selected} failed to join the envrionment."
                    )
    ...
    def initialize_orchestrator(self):
        self.environment["orchestrator"] = Orchestrator(
            preferences=self.preferences, environment=self.environment
        )
        ...
        if self.preferences["verbose"] >= 1:
            logger.debug(
                f"Information from envrionment: Orchestrator {orchestrator} joined the envrionment"
            )
            logger.debug(
                f"Orchestrator will deploy model: {orchestrator.orchestrator_model}"
            )
            logger.debug(
                f"Orchestrator have a validation dataset: {orchestrator.validation_set}"
            )
