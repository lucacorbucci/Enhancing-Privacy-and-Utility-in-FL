...
store_path = config.data_split_config.store_path
test_size = 0.2
...
for cluster in range(num_clusters):
    ...
    torch.save(cluster_test_data, f"{store_path}/test_cluster_{cluster}.pt")
    ...
    for client_id, client in enumerate(labels_and_group_indexes_public_private):
        ...
        torch.save(
            private_dataset,
            f"{store_path}/cluster_{cluster}_node_{client_id}_private_train.pt",
        )
        torch.save(
            private_dataset_validation,
            f"{store_path}/cluster_{cluster}_node_{client_id}_private_validation.pt",
        )
        ...
        torch.save(
            public_dataset,
            f"{store_path}/cluster_{cluster}_node_{client_id}_public_train.pt",
        )

        torch.save(
            public_dataset_validation,
            f"{store_path}/cluster_{cluster}_node_{client_id}_public_validation.pt",
        )
        ...
        torch.save(
            test_node_cluster,
            f"{store_path}/test_node_{client_id}_cluster_{cluster}.pt",
        )
...
torch.save(test_dataset, f"{store_path}/server_test_set.pt")
